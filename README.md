...
> | Module | Version |
> |--------|---------|
> | `dali2-core`, `dali2-adaptor` | **`dali_2.5.33`** |
> | `dali2-ui-foundation`, `dali2-ui-components` | **`dali-ui` `v2.5.32.10995`** — category-directory headers (`public-api/{views,types,configuration}/…`) and the JSON builder under `integration-api/builder` (`Dali::Ui::Integration::TreeNode`) |
>
> `dali-ui` trails `dali-core`/`dali-adaptor` by one minor version, so pair `dali-ui` 2.5.32
> with core/adaptor 2.5.33. Building against an older `dali-ui` that still keeps the builder
